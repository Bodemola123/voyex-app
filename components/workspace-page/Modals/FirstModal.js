...
function FirstModal({ closeModal, openModal, modalData, setModalData }) {
  ...
  const handleCategoryChange = (category) => {
    if (selectedCategories.includes(category)) {
      ...
      setModalData({ ...modalData, category: updatedCategories });
  ...
  const handleClose = () => {
    ...
    setModalData({ ...modalData, name: "", description: "", category: [] }); // Reset modal data
    closeModal();
  };
        ...
        <div className="flex justify-between items-center">
          ...
          <button
            ...
            className={`px-[21px] py-2.5 text-sm md:text-base bg-[#C088FB] text-[#0A0A0B] rounded-[25px] hover:scale-105 transition-all ${
              modalData.name?.trim() && modalData.description?.trim() && selectedCategories.length > 0
                ? ""
                : "cursor-not-allowed"
            }`}
          >
