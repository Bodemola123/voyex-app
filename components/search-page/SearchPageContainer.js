...
const fetchChatById = async (chat_id) => {
  ...
  3. Status Symbols: Owning real animals is a status symbol due to mass extinctions. Poor people resort to realistic electric robotic imitations of live animals, concealing their true nature from neighbors.`;
    ...
    const handleNewConversation = async () => {
      ...
      // Create a new chat via API to ensure we get a new chat_id
      const entityId = localStorage.getItem("entityId");
      const entityType = localStorage.getItem("userType") || localStorage.getItem("orgType");
      ...
      if (entityId && entityType) {
        const postBody = {
          entity_id: parseInt(entityId),
          entity_type: entityType,
          chat: [] // Empty chat array to start fresh
        };
        ...
        try {
          ...
          if (data.chat_id) {
            console.log("New chat started with chat_id:", data.chat_id);
            setChat({ chat_id: data.chat_id }); // Store the new chat ID
            localStorage.setItem("chat_id", data.chat_id); // Persist new chat_id in localStorage
          }
        } catch (err) {
          console.error("Error creating new chat:", err);
        }
      } else {
        console.error("Missing entity ID or type, cannot start new chat.");
      }
    };
