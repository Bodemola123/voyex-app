...
export default function TemplatePage() {
  ...
  // Convert step from URL param (string) to a number
    ...
    <div className="px-8 py-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex justify-between items-center px-6 py-4">
        ...
        <button 
          ...
          onClick={() => router.push("/template/1")}
        >
...
<div className="w-10 h-10 flex items-center justify-center border border-gray-500 rounded-md">
  ...
  <div className="w-2/3 space-y-4">
    {steps.map((step) => (
      <div
        key={step.id}
        onClick={() => router.push(`/template/${step.id}`)}
        className={`p-6 rounded-xl cursor-pointer transition-all duration-300 backdrop-blur-md border border-[#222222] hover:border-4 hover:border-[#C088FB] hover:shadow-lg ${
      ...
      >
        ...
        <button className="mt-4 px-4 py-2 bg-black bg-opacity-50 text-sm rounded-lg border border-[#222222] hover:bg-opacity-80 transition">
          View Full Details
        </button>
      </div>
    ))}
...
<div className="flex gap-6 mt-6 border-b border-[#222222]">
  {tabs.map((tab) => (
    <button
      key={tab}
      onClick={() => router.push(`/template/${currentStep}?tab=${tab}`)}
